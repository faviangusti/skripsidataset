...
def predict_image(model: YOLO, image: Image.Image, confidence: float):
    ...
    results = model.predict(source=image_bgr, conf=confidence, save=False, verbose=False)
    return results[0]
...
def predict_frame(model: YOLO, frame_bgr: np.ndarray, confidence: float):
    ...
    """
    results = model.predict(source=frame_bgr, conf=confidence, save=False, verbose=False)
    return results[0]
...
def camera_mode(model: YOLO, confidence: float) -> None:
    ...
    # Pola berikut adalah pendekatan standar yang direkomendasikan
    # streamlit-webrtc untuk menampilkan statistik live dari video
    # processor: memeriksa ctx.state.playing secara berkala dan membaca
    # detection_counts terbaru selama koneksi kamera masih aktif.
    while ctx.state.playing:
        ...
        summary_placeholder.info("Kamera belum aktif. Klik Start Camera untuk memulai.")
